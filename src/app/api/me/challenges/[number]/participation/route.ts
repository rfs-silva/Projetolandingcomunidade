...
export async function GET(
  ...
  try {
    ...
    const participation = await progressService.myChallengeParticipation(
      userId,
      number,
    )
...
export async function POST(
  ...
  try {
    ...
    const userId = await requireUserId()
    const { number } = await params
    const participation = await progressService.startChallenge(userId, number)
    return ok(participation, undefined, { status: 201 })
  } catch (error) {
    return handleError(error)
  }
}
